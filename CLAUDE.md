...
11. **Do not claim doneness, correctness, or resolution without observable evidence at the claim's granularity — the grounded-claims rule (no gaslighting).** Every assertion that something "works / is done / passes / is verified / is resolved / is fixed / is Sound" must be backed by evidence of *the actual behavior the claim asserts*, observed in *the assembled running system at the granularity claimed* — never inferred from code structure, a unit test, or a green gate that did not exercise the claimed behavior. This is the principle behind §5 (behavior over tautology tests), §19 (retrospective honesty), and §20 (gap-analysis); it is stated here as a Hard Rule because its violation is the project's most damaging recurring failure (M08.6 shipped "Sound, 0🔴"; IRL found 7🔴 — the assembled app was never run). Concretely:
    - **Claim–evidence parity.** Name the evidence; match the claim to it. A passing unit test licenses "the unit's logic is correct," NOT "the feature works." Reading code that *should* do X licenses *nothing* about runtime behavior — reading ≠ running. "The feature works" requires the assembled path to have been run and the behavior observed.
    - **"Resolved / fixed / FINAL DISPOSITION / closed" requires observed resolution in the running system.** A code change that *should* fix a finding is "fix written, unverified," not "resolved." (M08.G marked the budget cap "FINAL DISPOSITION"; it never persisted across restart — the prohibited pattern.)
    - **Verdicts must state what was NOT exercised, in the same breath.** A "green / Sound / passing" that did not run the assembled/real path says so explicitly ("unit + wire green; the assembled app was NOT run — execution unverified"). A verdict that omits its own blind spot is structurally untrusted.
    - **Structure is not behavior.** Never assert observable behavior from reading code. `save_framework` *containing* companion-write code ≠ companions are written (M08.6 #32: they were not).
    - **Uncertainty is stated, never smoothed.** "Unverified," "I have not run this," "I do not know whether X" are required wherever true. Implying confidence not held is the core of the prohibited pattern. This binds Claude's chat surfaces to the maintainer as much as the build machine's stage surfaces — a premature "PASS" before its full evidence is the same violation.
    - **Lead with the contradiction.** When evidence partly contradicts a doneness claim, the contradiction goes first — never buried after the happy path.

    A claim that cannot cite observable evidence at its granularity is not a status — it is a **hypothesis**, and must be labeled one. The cluster-gate pattern (BDD acceptance + mutation gate + per-cluster IRL), the zero-propagation rule, and the eval-first ladder are this rule's *enforcement mechanisms*; this rule is the *why*.
