...
# runtime-sandbox safety-primitive coverage (M05 Stage C1) — gate
# covers the plumbing files (validator + protocol + error). Excluded:
# (a) main.rs (binary stub), (b) lib.rs (run wrapping run_inner with
# the OS-signal future), (c) ipc.rs (cfg-platform accept-loop wrapper
# around the testable handle_connection seam — the OS-signal-class
# holdout for the sandbox crate). Stage C2 will lift the gate to
# include ipc.rs when seccomp / landlock / job_objects files land.
cargo llvm-cov --package runtime-sandbox \
    --ignore-filename-regex "src.main\.rs|generated|src.lib\.rs|src.ipc\.rs" \
    --fail-under-lines 95
```
