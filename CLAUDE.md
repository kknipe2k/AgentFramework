...
# runtime-sandbox safety-primitive coverage (M05 Stage C2; C1 added the
# gate, C2 lifted ipc.rs into it and added the OS-isolation modules).
# Excluded: (a) main.rs (binary stub), (b) lib.rs (run wrapping
# run_inner with the OS-signal future — parallel to drone's lib.rs
# exclusion). In-gate: validator.rs + protocol.rs + error.rs + ipc.rs
# + the platform isolation modules (seccomp.rs / landlock.rs on
# `cfg(target_os = "linux")`; job_objects.rs on `cfg(windows)`).
# Per-platform coverage attribution: Linux CI measures seccomp +
# landlock; Windows CI / Windows-local measures job_objects.
cargo llvm-cov --package runtime-sandbox \
    --ignore-filename-regex "src.main\.rs|generated|src.lib\.rs" \
    --fail-under-lines 95
```
