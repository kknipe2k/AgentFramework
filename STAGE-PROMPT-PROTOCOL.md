...
Children: `<assembled_run>` (the assembled test/path that proves the behavior — the dual-role BDD acceptance), `<irl_required>` (`true|false` — true for user-observable surfaces), `<mutation_gate>` (`cargo-mutants` on Rust clusters / Stryker on TS — a surviving mutant on the cluster's logic blocks close or is justified inline; **scoped per CLAUDE.md §5** — blocking on execution-wiring + critical modules, advisory elsewhere), `<cumulative_regression>` (v1.10 — append-on-close: the cluster's behavior enters the cumulative regression eval only once it closes, and every later close re-runs the accumulated prior-closed behavior; refines the Revision-3 timing decision — `docs/cluster-pattern.md` §9), `<triage>` (zero-propagation — new findings from the run are fix-in-cluster / new-cluster / explicit-ADR-scope-out, never routed forward; cluster-pattern.md §2).
...
<close_gate>
  ...
  <cumulative_regression>append-on-close: this rung's primitive joins the execution-regression suite; every later rung close re-runs the accumulated suite (cluster-pattern.md §9)</cumulative_regression>
...
- **v1.11 (enforcement — the deferral closes):** the v1.7 `<tdd_discipline strict="true">` → two-commit `<execution_steps>` coupling is now a CI **error** for `work_stage_prompt` blocks at Protocol version v1.11+ — the `<execution_steps>` MUST contain `red_phase_commit` + `surface_for_red_approval` + `green_phase_commit` + `surface_for_final_approval` when `<tdd_discipline strict="true">` is present (`checkStrictTddTwoCommit`). The deferral chain (v1.7→v1.8→v1.9, then unenacted through v1.9/v1.10) closed here: M06–M08.8 supplied the clean signal, **M08.9.B the negative case** (red-phase surface skipped under the collapsed single-`<implement>` `<execution_steps>` form — the §13 anti-pattern verbatim). Gated at v1.11 (encoded 111; v1.11 also fixed `detectProtocolVersion` to order two-digit minors) so M08.9 (v1.8) + all prior docs are grandfathered.
...
Validator behavior (v1.5 lean): structural — error if `<verification_passes>` is missing or empty; error if any `<pass>` child lacks the `name` attribute. The pass-name enum is documented (v1.12: six — `inventory` | `wire` | `behavior` | `multi_call_invariants` | `assembled_execution` | `adversarial`) but NOT code-enforced (the validator never carried the four-name enum in code — the fifth and sixth passes are additive, no validator change). The `adversarial` pass (v1.12) actively attempts hostile cases against the milestone's NEW/touched trust-boundary surfaces — boundary escape, malformed input, failure injection, granted-authority abuse — asserting the observed rejection + the absent side effect; canonical prose: `STAGE-V-VERIFIER-PROMPT-TEMPLATE.md`.
...
v1.12 — **A sixth verifier pass (`adversarial`) in `<verification_passes>` + the `STAGE-V-VERIFIER-PROMPT-TEMPLATE.md` six-pass update.** Standing adversarial verification of the milestone's NEW/touched trust-boundary surfaces — boundary escape (`..` traversal, symlink, scope-glob bypass, direct-`invoke` params no UI sends), malformed input (oversized/no-delimiter frames, truncated streams), failure injection (stalls — hang vs clean suspend; mid-stream disconnects; every error arm), and granted-authority abuse (the prompt-injection lens, `docs/SECURITY.md` "Prompt injection — posture"). Assertions on the OBSERVED rejection + the ABSENT side effect, never an error event alone; unattempted → labeled hypothesis (rule 11); unrunnable → 🟡 naming the blocker. Origin: the 2026-06-09 maintainer-commissioned external review's meta-finding — the IRL/cluster-gate culture verifies the happy path *really runs* but exercises nothing hostile; its eight Criticals (TD-050…057) all lived in that blind spot. The pass scopes per milestone via the template's `{{adversarial-targets}}` parameter (derived from X.2 tables filtered to boundary code — see the template's authoring guidance). Lean-validator pattern continued (the sixth pass name is additive; the pass-name enum remains documented-only — no validator change; first enforced use: M09.5.V).

v1.11 — **Enforcement promotion (no new tag) + a latent parser bug fix.** The v1.7 `<tdd_discipline strict="true">` → two-commit `<execution_steps>` coupling becomes a **code-enforced validator cross-check** — the first promotion OUT of the lean pass-through set (v1.3–v1.10 were all structural-only). The v1.7 changelog #4 deferred this "to v1.8+ once 2+ milestones show clean signal"; v1.8 §G re-deferred to v1.9; it then went unenacted through v1.9/v1.10. M06–M08.8 supplied the clean signal; **M08.9.B supplied the negative case** — its `<tdd_discipline strict="true">` carried a collapsed `<execution_steps><implement>` form with a single end `<approval_surface>`, so the build committed red+impl and surfaced once, skipping the red-phase surface gate (the §13 anti-pattern verbatim; the maintainer caught it by asking "did you surface red?").

1. **Validator cross-check (the promotion).** `bin/validate-stage-prompts.mjs::checkStrictTddTwoCommit`: a `work_stage_prompt` at Protocol version ≥ v1.11 declaring `<tdd_discipline strict="true">` whose `<execution_steps>` omits any of `red_phase_commit` / `surface_for_red_approval` / `green_phase_commit` / `surface_for_final_approval` is a CI **error**. This makes the §13 anti-pattern + the §7 "MUST be the two-commit shape" structurally unshippable, not merely documented.
2. **`detectProtocolVersion` parse fix.** The version banner was parsed `major + minor/10`, collapsing v1.10→2.0 and v1.11→2.1 (two-digit minors misorder). Now encoded `major*100 + minor` (v1.6→106, v1.10→110, v1.11→111); `simplify_pass`'s gate + the message formatting (`formatVersion`) updated to match. No behavior change for the single-digit-minor docs that exist.
3. **Grandfathering.** Gated at v1.11 (encoded 111) so M08.9 (v1.8) + all prior phase docs are exempt — validator PASS pre+post (101 stage-prompt blocks, 21 docs; a temp v1.11 strict-collapsed fixture confirmed the error fires, then was removed). The rule applies forward: M09+ (and any milestone re-bannered v1.11) author the explicit two-commit `<execution_steps>` natively. **No phase doc was edited to land this — M08.9 untouched** (maintainer constraint).
4. **Docs.** §7 slot validator-behavior note + §11 enforcement line + §13 anti-pattern all updated from "deferred / authoring-discipline" to "enforced v1.11+." Process change, not product (CLAUDE.md §20 — no gap-analysis entry).

v1.10 — One additive child element on the v1.9 `<close_gate>` slot — `<cumulative_regression>` — enacting the cumulative-regression / append-on-close close discipline (M08.7.X process hardening; `docs/cluster-pattern.md` §9). A close is done only when the cluster's own behavior runs AND all prior-closed behavior still runs; a behavior enters the cumulative regression eval only once its cluster closes (so it never tests an in-flight surface — this refines the `docs/protocol-revisions-irl-fail-rate.md` Revision-3 "build the cumulative gate after stabilization" timing into "build it incrementally, append-on-close"). Lean-validator continued: `<cumulative_regression>` is a `<close_gate>` child, structural pass-through (no allowlist change; no enforcement logic added — the appended eval / CI wiring is each milestone's close-gate work). Standing forward (M08.7.B → v0.1 ship), non-expiring.
