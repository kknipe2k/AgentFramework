...
**Inventory rows must be grep-verified at authoring time.** For each row in the inventory above AND for every file path claimed in any X.2 "Files to Change" table AND for every symbol / function / struct / IPC variant / integration point named in X.3 "Detailed Changes" narrative, the author runs `Test-Path` (Windows) / `ls` / `grep` / file inspection BEFORE shipping the Phase doc. The inventory section is **verified**, not **descriptive** — listing what the author *thinks* is there without running the verification is the M04.A2 failure mode (Phase doc claimed `event_translation.rs`, `prompt_template.rs`, `WriteSignal` IPC variant, `vdr.rs` in `runtime-main`, plus already-landed events claimed as new — none matched codebase reality; build agent halted at execution time). Trusting `agent-runtime-spec.md` text + `docs/gap-analysis.md` carry-forward chains as proxies for codebase state is a named failure mode per `docs/gotchas.md` #41 + STAGE-PROMPT-PROTOCOL.md §10 v1.3 hardening rule. Spec describes intent, not necessarily reality; gap-analysis is append-only and items "carry forward" but don't auto-close when work happens incidentally. Phase doc revisions that surface drift mid-execution are evidence the inventory was treated as descriptive; the right preventive is grep-verification at authoring time, not catch-and-revise mid-execution.
