...
| Role | Responsibility |
|---|---|
| **Claude** | Maintain the live observation log AS the session unfolds. At session end, score the retrospective, evaluate threshold gates, propose decisions for the next sub-milestone. Surface the filled-in retrospective alongside the PR description. |
...
|---|---|---|---|
| Per-sub-milestone retrospective | `docs/build-prompts/retrospectives/M[NN].[N]-retrospective.md` | Claude during/after the sub-milestone session | Filled live during work; finalized at session end alongside PR draft |
| Per-parent-milestone summary | `docs/build-prompts/retrospectives/M[NN]-summary.md` | Claude at the end of the last sub-milestone | Aggregates findings across the parent milestone's sub-milestone retrospectives |
...
## Why per-sub-milestone, not just per-parent-milestone

Per-sub-milestone retrospectives capture friction early. M01.1's retrospective (after ~5–8 hours) can surface a pattern problem before M01.2's session opens — saving 25+ hours of compounding error. A retrospective only at end of M01 (after all four sub-milestones) is a 30+ hour feedback loop. The per-sub-milestone cadence is the early-warning system.
...
Apply these gates after scoring. **All hard gates must pass for the next sub-milestone (or the next parent milestone) to proceed without protocol revision.**
...
- **G4: the milestone (sub-milestone) actually shipped.** PR merged to main; CI green; acceptance criteria all checked.
...
|---|---|---|
| All pass | All pass | **Pattern is sound.** Proceed to next sub-milestone (or next parent milestone). Apply minor revisions to CLAUDE.md based on Axis 3 notes. |
...
| Any hard gate fails | n/a | **Pattern is not yet ready.** Diagnose which gate failed and why. Fix the underlying issue, then re-run the failed sub-milestone (or run a recovery session) before proceeding. |
...
1. **All gates pass, scoring confirmed** — proceed to next sub-milestone in a fresh session.
