...
**Resolution status:** resolved (M08.9.A impl `3e4ee09` — closed at the M08.9.G closeout, 2026-06-06. `fold_outcome` now folds `AgentEvent::TierViolation` into a new `TestOutcome.tier_blocks: Vec<TierBlock>` and derives a 3-state `verdict: TestVerdict` (`Pass` / `Fail` / `TierLimited`) — `tester.rs:203–208` (the new match arm replacing the `_ => {}` drop) + `:222–229` (the verdict computation: defect ⇒ Fail; else tier-blocked ⇒ TierLimited; else Pass). `passed` is **unchanged** (caps + integrity only — a tier block is the user's tier setting, NOT a framework defect, exactly the "Why it's debt not a bug" reasoning + the "Recommended approach: do NOT fold TierViolation into capability_failures" guidance below; ADR-0030). The hand-written `TestOutcome` Rust struct + its hand-written TS mirror (`ipc.ts:599–660` — `TierBlock` interface + `TestVerdict` type + the two new fields) stay in lockstep, producer-driven per gotcha #94; no `schemas/` change, no ADR. `TesterModal` renders the 3-state verdict (`TesterModal.tsx:38–74`) — a Novice tier-blocked run reads **TIER-LIMITED** + the blocked action + a Promote link, not a green PASS — and M08.9.B added the verdict→tool-payload→raw drill-down. **Observed in the running app** (rule 11): M08.9.V's 5th assembled-execution pass drove the real Tauri binary — a Novice Write run rendered verdict `TIER-LIMITED` with `tester-result--tier-limited` + the Promote affordance and wrote no file; a Read run drilled a tool step to its input payload + Show-raw `tool_invoked`. The committed merge-blocking regressions (`tests/e2e-tauri/tester_verdict.e2e.ts` + `tester_drilldown.e2e.ts`) execute that assertion green only after M08.9.D.fix `61f1b11` closed V 🔴 #1 (the shared-session modal-scrim teardown — gotcha #95): key-present `test:e2e:tauri` 12/12, exit 0. The maintainer real-app IRL is the authoritative close. The separate phantom-`passed` verify finding (`vdr.rs:98`) is its own open [[TD-046]], not part of this entry. Entry text immutable; status updated in place per the TD-002/005/036 precedent.)
