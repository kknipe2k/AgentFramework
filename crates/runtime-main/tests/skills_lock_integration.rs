...
fn write_entry_appends_second_entry_preserving_first() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let path = lock_path(&dir);
    skills_lock::write_entry(
        &path,
        "alpha@1.0.0",
        url_entry(&skills_lock::content_hash(b"A")),
    )
    .expect("write #1");
    skills_lock::write_entry(
        &path,
        "beta@2.0.0",
        file_entry(&skills_lock::content_hash(b"B")),
    )
    .expect("write #2");
    ...
    assert!(
        v["installed"]["alpha@1.0.0"].is_object(),
        "first entry survived"
    );
    assert!(
        v["installed"]["beta@2.0.0"].is_object(),
...
fn write_entry_replaces_same_key_in_place() {
    ...
    skills_lock::write_entry(
        &path,
        "alpha@1.0.0",
        url_entry(&skills_lock::content_hash(b"old")),
    )
    .expect("write old");
    ...
    let v = serde_json::to_value(&lock).unwrap();
    assert_eq!(
        v["installed"]["alpha@1.0.0"]["content_hash"],
        json!(new_hash)
    );
    assert_eq!(
...
fn verify_happy_path_is_ok() {
    ...
    skills_lock::write_entry(
        &path,
        "pkg@1.0.0",
        url_entry(&skills_lock::content_hash(bytes)),
    )
    .expect("write");
...
fn verify_tamper_returns_hash_mismatch_carrying_sri_expected_and_actual() {
    ...
    match err {
        LockError::HashMismatch {
            ...
            assert_eq!(
                a, actual,
                "actual = the recomputed SRI hash of the drifted bytes"
            );
...
fn verify_unknown_artifact_returns_not_found() {
    ...
    let err = skills_lock::verify(&path, "ghost@9.9.9", b"x")
        .expect_err("unknown ref must not silently pass");
    assert!(
...
fn canonical_serialization_is_byte_identical_regardless_of_install_order() {
    ...
    skills_lock::write_entry(
        &p1,
        "zeta@9.0.0",
        file_entry(&skills_lock::content_hash(b"Z")),
    )
    .unwrap();
    skills_lock::write_entry(
        &p1,
...
fn generated_skills_lock_type_round_trips_a_spec_faithful_fixture() {
    ...
    assert_eq!(
        reser, fixture,
        "generated type is a lossless mirror of the schema shape"
    );
