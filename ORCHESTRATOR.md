...
- **Milestone:** **M08.8 — Design Workbench + Gap-Resume — IN PROGRESS** (branch `claude/m08.8-design-workbench`, unmerged; phase doc rewritten this cycle to **visual-foundation-first**). M08.8 makes the M08.7 engine **app-observable + human-operable**. Engine state (M08.7a, #120): **3 of 6 primitives execute** — built-in tools (E-01/02), skills (E-03 + real-model IRL), gap-**suspend** (E-04 + IRL); **3 paint** — sub-agents, plans, hooks (→ M08.9); budget executes engine-only (E-05). All flips observed **at engine granularity** (assembled evals + a few real-model IRLs), **not yet app-UI-observable** — that gap is M08.8. ✅ **The fabricated-fixture audit is COMPLETE — the 3/6 "executes" rows are VERIFIED CLEAN by two independent reads (build + orch):** E-01..E-05 are all REAL-SHAPE (stub only the provider; run the real executor/enforcer/loop; assert real side-effects). The `replay.rs` disease was localized — **fixed** (B.fix2) + one **latent** sibling (`vdr` verify arm, **TD-046**, dormant until M08.9 rung 8). The executes rows are NOT compromised; prevention = **gotcha #94** (producer-driven fixtures). [Prior: M08.6 #105; M08.7a #120.]
- **Last completed (M08.8, all on origin):** A (exec-view data plane, TD-034) → B (Light Instrument tokens + 3-pane shell + Toast/Modal) → **B.fix** (restyle every surface to the mockup + IBM Plex woff2 + TesterModal→Modal/TD-043) → **B.fix2** (closed **TD-044** reload-reconstruct: `replay.rs::translate_agent` read invented `payload.event` vs a serialized-`AgentEvent` log → 0 nodes; ~170 broken lines → ~10 lines direct serde; 5/5 reload + 8/8 e2e) — **all pushed** (merge `1f90b26`). **TD-045 fabricated-fixture audit DONE** (`66334c2`, two independent reads): E-01..E-05 REAL-SHAPE (executes rows clean); **TD-046** logged (🔴 latent — `vdr` verify phantom `passed`, owner M08.9 rung 8); drift-risk fixtures routed (recovery→D precond, projectors→rungs 7/8, ipc.ts→TD-011); prevention = **gotcha #94**. Earlier: M08.7a closeout (#120).
- **Next action:** the maintainer **"looks like the mockup, fully" walk closes B.fix** — the one open close-gate (automated `design_conformance` 5/5 + `builder_load_aria` deflaked are green; the subjective design pass is the maintainer's, per ADR-0021/rule 11). **Then M08.8, reprioritized order E → C → D → F** (each closes on a real **key-present** IRL — gotcha #94/rule 11: the injected-event e2e passes even if the real path is broken, so spend the hands-on run): **E** tier in the run loop FIRST (TD-036 — wires the user's tier so **Promoted / write-capable runs** work; `test_framework` hardcodes Novice today, so built workflows run read-only until E; mutation BLOCKING) → **C** budget visible → **D** gap resolve-and-resume (ADR-0029, reuses `recovery::resume`; **D precondition: producer-drive `recovery_lifecycle`'s snapshot-state-deser** — phase doc D.1; mutation BLOCKING) → **F** UX hardening + save-path 🔴 (#17/#22/#32). **Then M08.9** (sub-agents → plan → hooks [**+ TD-046 `vdr` fix before a verify producer wires**] → MCP-fix #23 → composition) → **M09 mentor (DEPRIORITIZED)**. Stage letters stay stable IDs (sections in A–F label order); **execution order is E → C → D → F** — E pulled ahead 2026-06-04 so write-capable runs land first (the pre-rewrite A–E was corrected at `fa4b635`).
- **Open threads:**
  - Claude Design / DESIGN.md / Stage D protocol scaffolding (master plan steps 10–12 — `STAGE-DESIGN-REVIEW-PROMPT-TEMPLATE.md`, `DESIGN-REVIEW-RETROSPECTIVE-TEMPLATE.md`, `docs/design-quality.md` append-only ledger, CLAUDE.md §19 retro extension, STAGE-PROMPT-PROTOCOL `<design_review_stage_prompt>` schema; the **first real Stage-D design-review run is M08.8** — the first design-heavy milestone, woven into each UI cluster against `DESIGN.md`).
  ...
  - The post-M08.6 findings routing (zero-propagation; **updated post-M08.7a #120**): execution primitives 1–5 **closed in M08.7a**; the remaining execution primitives (sub-agents / plan [`plan_loop` per **ADR-0026**] / hooks / MCP-fix #23 / composition) → **M08.9**; `TestOutcome.vdr` population stays **M9.A** (ADR-0026 — vdr is a Tester output field, not an execution primitive). The Builder 🔴/🟡 (#12 recovery, #17 template, #19 tier-desync, #22 budget-persist, #32 save-companions, validation/UX) → **M08.8** (the app-workbench absorbs them per the front-to-back re-scope — NOT M08.6.7). #31 ARIA narrowing → **M08.9** composition (grant/use split = v1.0 ADR).
