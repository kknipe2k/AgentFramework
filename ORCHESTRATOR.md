...
- **Next action:** **M08.7 in flight** — rungs 1–3 closed (built-in tools / capability scope-gate / skill-load; rungs 1+3 IRL-observed via RUST_LOG + live eval, rung 2 assembled + real-app-deferred TD-036); **rung 4 (gap) suspend half approved** (loop-break suspend-and-record, ADR-0028; live-eval close + the ADR-0028/TD/live-eval additions pending push); **resolve-and-resume PULLED FORWARD into v0.1** per maintainer 2026-06-01 — **ADR-0029** (best-practice-grounded). rungs 5 (budget) + N (ARIA) + a new **gap-resume rung** remain. **Claude Design DONE** — DESIGN.md visual layer merged (#117); the app-workbench design gate is CLEARED. **Phase 0a is CLOSED:** cluster-gate codified (`docs/cluster-pattern.md`) + zero-propagation wired (§3) + v1.9 protocol fold-in (`<close_gate>` + cluster-cycle step names + the Stage V 5th `assembled_execution` pass + V template) + M08.6 retro graduations applied (gotchas #88–91 + #75 clarification + the Stage-0 routing template slot) + Stryker spec'd (`cluster-pattern.md` §5; build-side install pending — NOT needed for the Rust M08.7a). **Pre-M9 plan (re-scoped 2026-06-01 per maintainer — v0.1 IS the working front-to-back runtime; M09 deprioritized):** the product is the complete **human-operable, no-AI** loop — **new-load → run → gap → human Grant/Install/Decline → resume → complete** — built front-to-back before the mentor. Sequence: **M08.7 Execution Engine** (the eval-first ladder; the *engine* RUNs each primitive + ARIA's v0.1 subset, observed via eval / RUST_LOG) **+ a new gap-resume rung** (runtime reload→deliver→continue per **ADR-0029**, assembled-tested with a stub resolution; the idempotent rebuild §1b — no re-execution of completed calls — is the load-bearing assertion) → **the app-workbench milestone** (build/load/run/watch/validate/test *in the app* **+ the Grant/Install/Decline gap-resolution UI = the non-AI troubleshoot**; absorbs the runtime-exposure TDs **TD-034** (agent output / live-graph execution view), **TD-036** (tier in the run loop), **TD-037** (canvas skill bodies) **plus** the 32 Builder findings; design gate CLEARED — DESIGN.md visual layer #117) = **the complete front-to-back runtime ships here** → **M9 mentor (DEPRIORITIZED)** — the AI-assist layer on top of the human-operable loop; nothing to mentor until the loop works. resolve-and-resume architecture = **ADR-0029** (durable checkpoint-resume + idempotent rebuild + governed human re-authorization; web-best-practice-grounded, not improvised).
- **Open threads:**
  ...
  - The post-M08.6 findings are **clusters in M08.7 / M08.6.7, NOT M9 intake** (zero-propagation): execution gaps (built-in tools / sub-agents / skills / gaps / plans / hooks — incl. `plan_loop` production wiring → M08.7 **rung 7** per **ADR-0026**; `TestOutcome.vdr` population stays **M9.A** per ADR-0026 — vdr is not an M08.7 rung, the earlier "vdr rung 6" note was a mislabel since rung 6 is sequential spawn) → M08.7; Builder 🔴/🟡 (#12 recovery, #17 template, #19 tier-desync, #22 budget-persist, #23 MCP-spawn, #32 save-companions, validation/UX) → M08.6.7. #31 narrowing = strict-narrowing-fix-ARIA at M08.7 rung N (grant/use split = v1.0 ADR).
