...
- **Next action:** **M08.7 in flight** — rungs 1–3 closed (built-in tools / capability scope-gate / skill-load; rungs 1+3 IRL-observed via RUST_LOG + live eval, rung 2 assembled + real-app-deferred TD-036); **rung 4 (gap / `request_capability`) at red** (loop-break suspend-and-record; resolve-and-resume → M09, ADR-0028); rungs 5 (budget) + N (ARIA) remain. **Run Claude Design NOW** (parallel — it gates the app-workbench UI build, not the M08.7 engine). **Phase 0a is CLOSED:** cluster-gate codified (`docs/cluster-pattern.md`) + zero-propagation wired (§3) + v1.9 protocol fold-in (`<close_gate>` + cluster-cycle step names + the Stage V 5th `assembled_execution` pass + V template) + M08.6 retro graduations applied (gotchas #88–91 + #75 clarification + the Stage-0 routing template slot) + Stryker spec'd (`cluster-pattern.md` §5; build-side install pending — NOT needed for the Rust M08.7a). **Pre-M9 plan (re-scoped 2026-05-31 per maintainer — app-workbench-before-mentor):** Phase 0 (0a done; **0b DESIGN.md visual via Claude Design — user-driven; runs NOW in parallel with the M08.7 tail; it gates the app-workbench UI build**) → **M08.7 Execution Engine** (the eval-first ladder; the *engine* RUNs each primitive + ARIA's v0.1 subset, observed via eval / RUST_LOG — **not yet exposed in the app**) → **the app-workbench milestone** (was "M08.6.7 Builder correctness"; **EXPANDED** per maintainer: the app does **build → load → run → validate → test** any framework end-to-end *in the app*, with real-app IRL at each step — absorbing the runtime-exposure TDs **TD-034** (agent output / live-graph execution view), **TD-036** (tier wired into the run loop), **TD-037** (canvas-authored skill bodies) **plus** the 32 Builder findings; this is where "ARIA runs *in the app* and you watch it" lands, vs M08.7's engine-level run; **gated on the Claude Design pass**) → **M9 mentor** (begins only once the app is a working workbench — the mentor has nothing to mentor until you can build/run/validate frameworks in the app).
- **Open threads:**
