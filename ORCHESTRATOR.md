...
- **Next action:** finish Phase 0a (Stryker build-side install + the `STAGE-PROMPT-PROTOCOL.md` cluster-gate fold-in + the Stage V 5th-pass template), then M08.7a rung 1 (build-side). **Pre-M9 plan (confirmed):** Phase 0 foundations (0a cluster-gate codify — in progress; 0b DESIGN.md visual via Claude Design — user-driven, before the Builder UI track) → **M08.7 Execution Engine** (the eval-first ladder; make the runtime RUN; ends at *ARIA runs* the v0.1 subset) → **M08.6.7 Builder correctness** (the 32 findings as clusters) → **M9 mentor**.
- **Open threads:**
