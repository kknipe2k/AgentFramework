...
- **Milestone:** **M08.8 — Design Workbench + Gap-Resume — IN PROGRESS** (branch `claude/m08.8-design-workbench`, unmerged; phase doc rewritten this cycle to **visual-foundation-first**). M08.8 makes the M08.7 engine **app-observable + human-operable**. Engine state (M08.7a, #120): **3 of 6 primitives execute** — built-in tools (E-01/02), skills (E-03 + real-model IRL), gap-**suspend** (E-04 + IRL); **3 paint** — sub-agents, plans, hooks (→ M08.9); budget executes engine-only (E-05). All flips observed **at engine granularity** (assembled evals + a few real-model IRLs), **not yet app-UI-observable** — that gap is M08.8. ⚠️ **B.fix2 exposed a fabricated-fixture class bug** (`replay.rs` translator read invented field-names, was tautology-tested against a shape that never existed, **always broken vs real signals**); **TD-045** sweeps for siblings — until that audit runs, the 3/6 "executes" carry an asterisk (*do the evals feed REAL serialized shapes?*). [Prior: M08.6 #105; M08.7a #120.]
- **Last completed (M08.8, unpushed tail on the branch):** A (exec-view data plane, TD-034) → B (Light Instrument tokens + 3-pane shell + Toast/Modal) → **B.fix** (restyle every surface to the mockup + IBM Plex woff2 + TesterModal→Modal/TD-043) — both **pushed**; → **B.fix2** (closed **TD-044** reload-reconstruct: root cause = `replay.rs::translate_agent` read invented `payload.event` field-names vs a serialized-`AgentEvent` signal log → 0 nodes; ~170 broken lines → ~10 lines direct serde; 5/5 reload + 8/8 e2e) **committed, NOT pushed**; **TD-045** (🟡) logged for the fabricated-fixture sweep. Earlier: M08.7a closeout (#120 — summary + immutable gap-analysis + ADR-0027/0028).
- **Next action:** (1) **push B.fix2** (resolve the TD-044 ledger conflict → RESOLVED); (2) **TD-045 audit** — sweep the signal translators (`replay.rs` siblings; `vdr`/`plan_projector`/`token_usage`) AND the execution evals (E-01..E-05 + `recovery_lifecycle.rs`) for the same fabricated-fixture disease — the *is-the-engine-real* check that gates confidence in 3/6 "executes" (do it before more C–F paint); (3) the maintainer "looks like the mockup, fully" walk closes B.fix. **Then M08.8 C–F** (each closes on a real key-present IRL per ADR-0021): **C** budget visible → **D** gap resolve-and-resume (ADR-0029, reuses `recovery::resume` — itself a TD-045 audit target; mutation BLOCKING) → **E** tier in run loop (TD-036; mutation BLOCKING) → **F** UX hardening + save-path 🔴 (#17/#22/#32). **Then M08.9** (sub-agents → plan → hooks → MCP-fix #23 → composition) → **M09 mentor (DEPRIORITIZED)**. NOTE: this §9 staging was **stale** (pre-rewrite A–E with budget@B/gap@C/tier@D); corrected here to the rewritten **A / B / B.fix / C / D / E / F**.
- **Open threads:**
