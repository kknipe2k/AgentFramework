...
- **Next action:** the maintainer **"looks like the mockup, fully" walk closes B.fix** — the one open close-gate (automated `design_conformance` 5/5 + `builder_load_aria` deflaked are green; the subjective design pass is the maintainer's, per ADR-0021/rule 11). **Then M08.8 C–F** (each closes on a real **key-present** IRL — gotcha #94/rule 11: the injected-event e2e passes even if the real path is broken, so spend the hands-on run): **C** budget visible → **D** gap resolve-and-resume (ADR-0029, reuses `recovery::resume`; **D precondition: producer-drive `recovery_lifecycle`'s snapshot-state-deser per the audit** — phase doc D.1; mutation BLOCKING) → **E** tier in run loop (TD-036 — the "Promote" production-wiring gap; mutation BLOCKING) → **F** UX hardening + save-path 🔴 (#17/#22/#32). **Then M08.9** (sub-agents → plan → hooks [**+ TD-046 `vdr` fix before a verify producer wires**] → MCP-fix #23 → composition) → **M09 mentor (DEPRIORITIZED)**. Staging is the rewritten **A / B / B.fix / C / D / E / F** (the earlier pre-rewrite A–E was corrected at `fa4b635`).
- **Open threads:**
