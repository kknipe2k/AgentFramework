...
- **Milestone:** **M08.7a merged (PR #120)** — the execution engine: rungs 1–5 (built-in Read/Write execute + feed back / capability-on-live-tool / skill-load injects into context / gap suspends cleanly / budget enforces at the run loop); **ADR-0027** (skill-into-context injection) + **ADR-0028** (built-in tool execution contract) Accepted; M08.7.V Sound 0🔴. The single-agent engine *executes*, observed in the assembled engine (evals E-01..E-05) + per-rung IRL — the paints→executes flip is real **at engine granularity**. [Prior: M08.6 #105; post-M08.6 docs #106; DESIGN.md visual layer #117; front-to-back re-scopes #118/#119.]
- **Last completed:** M08.7a closeout (M08.7-summary + the **immutable gap-analysis M08.7 entry** + simplify_pass + coverage reconciliation [no gate change; runtime-main 96.23% ≥95] + ADR-0027/0028 flips) merged via #120, plus 3 CI-unblock fixes (vitest 2→4 npm-audit; `.prettierignore` + eslint ignores for #117's design mockups). The M08.6-IRL **7🔴 recorded with OBSERVED dispositions** — 0 closed by M08.7a (out of execution scope), 5→M08.8, 2→M08.9. The app does **not** yet *show* execution (RUST_LOG-only; budget invisible; gap dead-ends) — that is the M08.8 job.
- **Next action:** **M08.8 — Design Workbench + Gap-Resume** (phase doc authored: `docs/build-prompts/M08.8-design-workbench.md`). The front-to-back app where execution becomes **app-observable** + the runtime becomes **human-operable** (new-load → run → **watch** → gap → **Grant/Decline** → resume → complete), built to **`DESIGN.md`** (#117). Unifying insight: DESIGN.md's 9 interaction principles each trace to a Builder finding — **building to DESIGN.md IS resolving the 32 findings**. Staging (small→large, each real-app `tauri-driver` IRL'd per ADR-0021): **A** live-graph execution view (TD-034) → **B** budget visible (closes the rung-5 in-app-observe deferral — live-vs-scoped store reconcile + auto-apply cap + dead-modal fix) → **C** gap resolve-and-resume (**ADR-0029** engine resume — idempotent §1b rebuild, the load-bearing assertion — + the Grant/Install/Decline UI) → **D** tier in the run loop (TD-036) + tier-UI truth (#19/#20) → **E** UX hardening + save-path 🔴 (#17/#22/#32 + the 🟡/🟢 clusters, distributed by surface). **Stage A authored at entry next.** Scope calls (best-practice, maintainer-approved): **one milestone, findings distributed by surface** (not split); **TD-037 stays M09 Generators**. Then **M08.9** (sub-agents → plan [`plan_loop` per ADR-0026] → hooks → MCP-fix #23 → composition + #31 ARIA) → **M09 mentor (DEPRIORITIZED)** — the AI-assist layer on top of the operable loop. The gap-**suspend** (rung 4, shipped #120) is **step 1 of ADR-0029**, NOT a separate ADR — earlier "ADR-0028 = gap-suspend" refs were stale (0028 is now the built-in-execution-contract).
- **Open threads:**
