...
- **Last completed (M08.8, all on origin):** A (exec-view data plane, TD-034) → B (Light Instrument tokens + 3-pane shell + Toast/Modal) → **B.fix** (restyle every surface to the mockup + IBM Plex woff2 + TesterModal→Modal/TD-043) → **B.fix2** (closed **TD-044** reload-reconstruct: `replay.rs::translate_agent` read invented `payload.event` vs a serialized-`AgentEvent` log → 0 nodes; ~170 broken lines → ~10 lines direct serde; 5/5 reload + 8/8 e2e) — **all pushed** (merge `1f90b26`). **TD-045 fabricated-fixture audit DONE** (`66334c2`, two independent reads): E-01..E-05 REAL-SHAPE (executes rows clean); **TD-046** logged (🔴 latent — `vdr` verify phantom `passed`, owner M08.9 rung 8); drift-risk fixtures routed (recovery→E precond, projectors→rungs 7/8, ipc.ts→TD-011); prevention = **gotcha #94**. Earlier: M08.7a closeout (#120).
- **Next action:** the maintainer **"looks like the mockup, fully" walk closes B.fix** — the one open close-gate (automated `design_conformance` 5/5 + `builder_load_aria` deflaked are green; the subjective design pass is the maintainer's, per ADR-0021/rule 11). **Then M08.8, alpha order C → D → E → F** (each closes on a real **key-present** IRL — gotcha #94/rule 11: the injected-event e2e passes even if the real path is broken, so spend the hands-on run): **C** tier in the run loop FIRST (TD-036 — wires the user's tier so **Promoted / write-capable runs** work; `test_framework` hardcodes Novice today, so built workflows run read-only until C; mutation BLOCKING) → **D** budget visible → **E** gap resolve-and-resume (ADR-0029, reuses `recovery::resume`; **E precondition: producer-drive `recovery_lifecycle`'s snapshot-state-deser** — phase doc E.1; mutation BLOCKING) → **F** UX hardening + save-path 🔴 (#17/#22/#32). **Then M08.9** (sub-agents → plan → hooks [**+ TD-046 `vdr` fix before a verify producer wires**] → MCP-fix #23 → composition) → **M09 mentor (DEPRIORITIZED)**. Stages **re-lettered alphabetical 2026-06-04** (tier→C, budget→D, gap→E) so letters match execution order; the §C–§F sections appear in order (the pre-rewrite A–E + the E-reprioritization were corrected at `fa4b635`/`618f97b`).
- **Open threads:**
